...
*- this Foxpro program NOTVFOUC.PRG (orig: NONEDIPS.PRG)
...
*- PART 3: UCC128 part of the label - VF OUTLET (7/5/2007)
*- ----------------------------------------------------------------------------------------------------------------------- -*
...
*- Zone B (Ship to [DC])
*- 14 August 2007 - Michael J. Sabal
*- Due to VF Outlet's unique requirements, Zone B must contain the consolidator address with
*- the specific store name.  The consolidator number cannot be stored in the store master
*- as is usually done, because they also need their ASNs separated by store.
*- We have decided to use the customer sales reference:region code to store the consolidator number,
*- and the ship thru table to store the consolidator address.
*- xxxxxAll orders shipping from PA should be consolidated to Gilbert (consol #1) regardless of region.xxxxx
*- 3 September 2008 - Michael J. Sabal
*- Routing changes:
*- Volume routing is unpredictable regardless of location
*- All normal TPE regions go to Hawthorne, CA via Averitt (consol 3)
*- All normal PA regions 2-4 go to Charlotte, NC via Averitt (consol 2)
*- All normal PA region 1 go direct to store

*- All addresses direct to store per VFO Routing 12/12/2018

lcConsolNum = vl_consolidator('VFOUT',store)
&& if (alltrim(location)='WHV' or alltrim(location)='WPAFG') then
&&   if lcConsolNum<>'1' then
&&     lcC1 = vl_consol_addr('VFOUT','2')
&&   endif
&&   *- direct to store for region 1
&& else  && TPE
&&   IF lcConsolNum<>'3' THEN 
&&   *lcC1 = vl_consol_addr('VFOUT','3')
&&     lcC1 = vl_consol_addr('VFOUT','2') && for 2/3/09 shipment
&&   ENDIF 
&& endif
...
lcZip = alltrim(sfsship_zip)
if LEN(lcZip)>5 then
  lcZip = substr(lcZip,1,5)
endif
lcStringBlock3 = lcStringBlock3 + '#T188.0#J'+str(lnOffset+55)+'#YB15/2O/3.5/5///420'+lcZip+'#G'
lcStringBlock3 = lcStringBlock3 + '#T178.0#J'+str(lnOffset+59)+'#YT107/2///(420)'+lcZip+'#G'

*- Zone D (Carrier, etc.)

lcStringBlock3 = lcStringBlock3 + '#T120.0#J'+str(lnOffset+55)+'#YT106/2///R'+lcConsolNum+'#G'
...
lcStringBlock3 = lcStringBlock3 + '#T43.0#J'+str(lnOffset+8)+'#YT105/2///'+'STORE:'+'#G'
lcStringBlock3 = lcStringBlock3 + '#M2/3#T43.0#J'+str(lnOffset+25)+'#YT109/2///'+alltrim(lcStore)+'#G#M1/1'
*lcStringBlock3 = lcStringBlock3 + '#T95.0#J'+str(lnOffset+17)+'#YT105/2///'+'PROD:'+'#G'
*lcStringBlock3 = lcStringBlock3 + '#T80.0#J'+str(lnOffset+17)+'#YT108/2///'+alltrim(sfsdept_name)+'#G'
...
*-----------------------------------------------------------------------------
*- FUNCTION vl_consolidator(customer,store)
*- Returns the region code from zzxcslsr
*- Michael J. Sabal, Notations, Inc. 14 Aug 2007
*-----------------------------------------------------------------------------
FUNCTION vl_consolidator 
PARAMETERS parm_customer, parm_store

  LOCAL lcSQLQuery, llCount, lcSQLReturn

  lcSQLQuery = "SELECT CST_REGION FROM zzxcslsr WHERE customer='"+alltrim(parm_customer)+"'"+ ;
               " AND store='"+alltrim(parm_store)+"'"
  = v_SQLPrep(lcSQLQuery,'RegionCode','')
  if used('RegionCode') and reccount('RegionCode')>0
    *- We have a valid range style
    lcSQLReturn = RegionCode.CST_REGION
  else
    lcSQLReturn = ""
  endif
...
*-----------------------------------------------------------------------------
*- FUNCTION vl_consol_addr(customer,consolidator#)
*- Returns the appropriate address from zzxdistr
*- Michael J. Sabal, Notations, Inc. 14 Aug 2007
*-----------------------------------------------------------------------------
FUNCTION vl_consol_addr 
PARAMETERS parm_customer, parm_consol

  LOCAL lcSQLQuery, llCount, lcSQLReturn

  lcSQLQuery = "SELECT center_name,address1,address2,city,state,zipcode FROM zzxdistr WHERE customer='"+alltrim(parm_customer)+"'"+ ;
               " AND center_code='"+alltrim(parm_consol)+"' AND center_type='C'"
  = v_SQLPrep(lcSQLQuery,'ConsolAddr','')
  if used('ConsolAddr') and reccount('ConsolAddr')>0
    *- We have a valid dc
    replace sfsship_addr1 with ConsolAddr.address1 in CurPart2Final
    replace sfsship_addr2 with ConsolAddr.address2 in CurPart2Final
    replace sfsship_city with ConsolAddr.city in CurPart2Final
    replace sfsship_state with ConsolAddr.state in CurPart2Final
    replace sfsship_zip with ConsolAddr.zipcode in CurPart2Final
