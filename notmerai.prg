...
*- this Foxpro program NOTMERUC.PRG (orig: NOTPCLUC.PRG)
...
*- PART 3: UCC128 part of the label - MERVYN (5/11/2005)
*- ----------------------------------------------------------------------------------------------------------------------- -*
...
scan
	...
	*- -----------------------------------------------------------------------------------------------
    lcSKU_UPC = vl_rangestyle(cur2Temp.ord_num,cur2Temp.pick_num,cur2Temp.Style,cur2Temp.color_code,cur2Temp.lbl_code)
    liStdPack = vl_stdpack(lcSKU_UPC)
	IF EMPTY(lcSKU_UPC)
...
lcStringBlock3 = lcStringBlock3 + '#T101.6#J'+str(lnOffset+30)+'#YL0/1/1/45#G'
...
if empty(center_code) then
  lcStringBlock3 = lcStringBlock3 + '#T155.0#J'+str(lnOffset+9)+'#YT108/2///'+alltrim(sfsship_name)+' DC '+alltrim(store)+'#G'
else
  lcStringBlock3 = lcStringBlock3 + '#T155.0#J'+str(lnOffset+9)+'#YT108/2///'+alltrim(sfsship_name)+' DC '+alltrim(center_code)+'#G'
endif
lcStringBlock3 = lcStringBlock3 + '#T155.0#J'+str(lnOffset+13)+'#YT108/2///'+alltrim(sfsship_addr1)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T155.0#J'+str(lnOffset+17)+'#YT108/2///'+alltrim(sfsship_addr2)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T155.0#J'+str(lnOffset+21)+'#YT108/2///'+ ;
	alltrim(sfsship_city)+', '+alltrim(sfsship_state)+'  '+alltrim(sfsship_zip)+'#G'

*- Zone C (Ship to Post [UCC128])

lcStringBlock3 = lcStringBlock3 + '#T188.0#J'+str(lnOffset+50)+'#YB15/2O/2.5/5///420'+alltrim(sfsship_zip)+'#G'
...
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+62)+'#YT108/2///'+'PO: '+alltrim(po_num)+;
	'      DEPT: '+alltrim(department)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+66)+'#YT108/2///'+'SKU: '+alltrim(SKU_UPC)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+70)+'#YT108/2///'+'STANDARD PACK: '+alltrim(str(liStdPack))+'#G'
lcStringBlock3 = lcStringBlock3 + '#T150.0#J'+str(lnOffset+70)+'#YT108/2///MIXED SKU#G'
...
*-----------------------------------------------------------------------------
*- FUNCTION vl_stdpack(range_style)
*- Returns the range style case pack from zzxrangh
*- Michael J. Sabal, Notations, Inc. 15 June 2007
*-----------------------------------------------------------------------------
FUNCTION vl_stdpack  
PARAMETERS parm_rangestyle

  LOCAL lcSQLQuery, llCount, lcSQLReturn

  lcSQLQuery = "SELECT rng_qty FROM zzxrangh WHERE rng_style='"+alltrim(parm_rangestyle)+"'"
  = v_SQLPrep(lcSQLQuery,'RangeStyle','')
  if used('RangeStyle') and reccount('RangeStyle')>0
    *- We have a valid range style
    lcSQLReturn = RangeStyle.rng_qty
  else
    lcSQLReturn = 0
  endif
