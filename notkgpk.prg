...
*- this Foxpro program NOTkgUC.PRG (orig: NOTPCLUC.PRG)
...
*- PART 3: UCC128 part of the label - K&G MENS CORP. (04/01/05)
*-
*- Updates 2017-04-07:
*- 1) Ship mode must be taken from the EDI Data warehouse REF*PHC
*- 2) All store and DC codes are now 4 digits
*- 3) For ship modes 2,3, and 8, an item number barcode must appear in zone F.
*- ----------------------------------------------------------------------------------------------------------------------- -*
...
scan
	...
	*=== TR 1012146 - BD - End of modification 4	
	...
	vl_sizer(lcDivision,'','curSizeHeader',lcSize_Code)
		...
		cur1GroupBySKU.Style,cur1GroupBySKU.color_code,cur1GroupBySKU.lbl_code,cur1GroupBySKU.dimension))
*        replace CurPart1.hilo_retail with lcRetail in CurPart1
...
scan
	...
	*- -----------------------------------------------------------------------------------------------
    ...
    IF EMPTY(lcSKU_UPC)
	*- For K&G, if there is no cust-style cross-reference entry for the label code, check for the same
	*- style/color under blank label code.
      lcSKU_UPC = vl_cstdr(cur2Temp.customer,'CUST_STYLE','',cur2Temp.division,cur2Temp.Style,cur2Temp.color_code,'','',cur2Temp.sizebucket)
    ENDIF	
	IF EMPTY(lcSKU_UPC)
...
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+9)+'#YT102/2///'+'140 N. ORANGE AVENUE'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+12)+'#YT102/2///'+'CITY OF INDUSTRY, CA'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T170.0#J'+str(lnOffset+15)+'#YT102/2///'+'91744'+'#G'
...
ENDIF
IF AT('#',sfsmark_addr1)>0 AND AT('##',sfsmark_addr1)=0 THEN
  lipos = AT('#',sfsmark_addr1)
  sfsstore_name = SUBSTR(sfsmark_addr1,1,lipos-1)+'#'+SUBSTR(sfsmark_addr1,lipos,(LEN(sfsmark_addr1)-lipos)+1)
ENDIF
IF AT('#',sfsship_addr1)>0 AND AT('##',sfsship_addr1)=0 THEN
  lipos = AT('#',sfsship_addr1)
  sfsstore_name = SUBSTR(sfsship_addr1,1,lipos-1)+'#'+SUBSTR(sfsship_addr1,lipos,(LEN(sfsship_addr1)-lipos)+1)
ENDIF
IF NOT EMPTY(center_code) then
...
ELSE
  lcStringBlock3 = lcStringBlock3 + '#T158.0#J'+str(lnOffset+2)+'#YT106/2///'+'TO:'+'#G'
  lcStringBlock3 = lcStringBlock3 + '#T155.0#J'+str(lnOffset+9)+'#YT106/2///'+alltrim(sfsstore_name)+'#G'
  lcStringBlock3 = lcStringBlock3 + '#T155.0#J'+str(lnOffset+12)+'#YT106/2///'+alltrim(sfsmark_addr1)+'#G'
  lcStringBlock3 = lcStringBlock3 + '#T155.0#J'+str(lnOffset+15)+'#YT106/2///'+alltrim(sfsmark_addr2)+'#G'
  lcStringBlock3 = lcStringBlock3 + '#T155.0#J'+str(lnOffset+18)+'#YT106/2///'+ ;
...
IF len(ALLTRIM(sfsship_zip))>5
  lcStringBlock3 = lcStringBlock3 + '#T188.0#J'+str(lnOffset+45)+'#YB15/2O/2.5/5///420'+substr(sfsship_zip,1,5)+'#G'
  lcStringBlock3 = lcStringBlock3 + '#T178.0#J'+str(lnOffset+49)+'#YT107/2///(420)'+substr(sfsship_zip,1,5)+'#G'
ELSE
  lcStringBlock3 = lcStringBlock3 + '#T188.0#J'+str(lnOffset+45)+'#YB15/2O/2.5/5///420'+alltrim(sfsship_zip)+'#G'
...
lcShipMode = alltrim(vl_edidatawarehouse(ord_num,0,'REF','PHC'))
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+54)+'#YT107/2///'+'PO: '+alltrim(po_num)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+58)+'#YT107/2///'+'DIV: '+alltrim(department)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+62)+'#YT107/2///'+'SHIP MODE: '+alltrim(lcShipMode)+'#G'

*- Zone F (if not combined with Zone E)

IF lcShipMode='2' OR lcShipMode='3' OR lcShipMode='8' THEN
	lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+66)+'#YT107/2///'+'COLOR: '+alltrim(color_code)+'#G'
	lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+70)+'#YT107/2///'+'SIZE: '+alltrim(size_desc)+ ;
		'   QTY: '+alltrim(str(lnTotalBlockQty))+'#G'
	lcStringBlock3 = lcStringBlock3 + '#T150.0#J'+str(lnOffset+55)+'#YT107/2///'+'ITEM CODE:'+'#G'
	lcStringBlock3 = lcStringBlock3 + '#T150.0#J'+str(lnOffset+70)+'#YB15/2M/2/4///'+alltrim(SKU_UPC)+'#G'
ENDIF

*- Zone G (Mark for store # [UCC128])

lcStringBlock3 = lcStringBlock3 + '#T95.0#J'+str(lnOffset+8)+'#YT106/2///'+'CARTON:'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T95.0#J'+str(lnOffset+20)+'#YT109/2///____ of ____#G'

*- Zone H (Mark for store # [text])

lcStore = alltrim(store)
IF LEN(lcStore)=3 THEN
  lcStore = '0'+lcStore
ENDIF
lcStringBlock3 = lcStringBlock3 + '#T45.0#J'+str(lnOffset+8)+'#YT106/2///STORE NUMBER:#G'
lcStringBlock3 = lcStringBlock3 + '#M1/2#T45.0#J'+str(lnOffset+20)+'#YT108/2///'+alltrim(lcStore)+'#G#M1/1'
