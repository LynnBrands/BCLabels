...
*- this Foxpro program CATO.PRG (orig: NOTPCLUC.PRG)
...
*- PART 2: UCC128 part of the label - CATO (4/17/2018)
*- ----------------------------------------------------------------------------------------------------------------------- -*
LOCAL lnIndLineBlock1,lnIndLineBlock2,lnIndLineBlock3
LOCAL lnOrgArea
LOCAL lnPageNum, lnLineNum, lnOffset
LOCAL lcShipTo
LOCAL lcDivision,lcStyle,lcFieldName,lnIndbucket,lcIndbucket, lnAggVal, lcSKU_UPC
...
LOCAL lcSortStyle,pack_description,casedescription
...
liCountCurGroupByCarton = reccount('curGroupByCarton')
liIterationCounter = 0	  
SCAN
liIterationCounter = liIterationCounter + 1
*- Group by carton_num
			...
			*- Bucket quantities
			...
			for lnIndbucket=2 to 11
				lcIndbucket=TRANS(lnIndbucket,"@L 99")
					...
					lnIndValidBucket=lnIndValidBucket+1
				if eval('curPart1.siz'+lcIndbucket+'_qty')>0 and lnIndValidBucket<10
					casedescription = casedescription + alltrim(eval('curPart1.siz'+lcIndbucket+'_head')) + ;
						":"+allt(str(eval('curPart1.siz'+lcIndbucket+'_qty')))+" #G"
...
lcStringBlock1 = lcStringBlock1 + '#T50.0#J'+str(lnOffset+65)+'#YT101/2///'+alltrim(str(liIterationCounter))+' of '+alltrim(str(liCountCurGroupByCarton))+'#G'	
...
IF len(ALLTRIM(sfsship_zip))>5
  lcStringBlock3 = lcStringBlock3 + '#T188.0#J'+str(lnOffset+50)+'#YB15/2O/3.5/5///420'+substr(sfsship_zip,1,5)+'#G'
  lcStringBlock3 = lcStringBlock3 + '#T178.0#J'+str(lnOffset+54)+'#YT107/2///(420)'+substr(sfsship_zip,1,5)+'#G'
ELSE
  lcStringBlock3 = lcStringBlock3 + '#T188.0#J'+str(lnOffset+50)+'#YB15/2O/3.5/5///420'+alltrim(sfsship_zip)+'#G'
  lcStringBlock3 = lcStringBlock3 + '#T178.0#J'+str(lnOffset+54)+'#YT107/2///(420)'+alltrim(sfsship_zip)+'#G'
ENDIF

*- Zone D (Carrier, etc.)
&& lcCartonID = vl_poline(pick_num,line_seq)
lcCartonID = lcSKU_UPC
lcStringBlock3 = lcStringBlock3 + '#T138.0#J'+str(lnOffset+30)+'#YT108/2///CARTON ID#G'
lcStringBlock3 = lcStringBlock3 + '#T138.0#J'+str(lnOffset+35)+'#YT108/2///'+alltrim(lcCartonID)+'#G'

*- Zone E (PO / Dept)

lcColor2 = cust_color(customer,style,color_code)
if len(alltrim(lcColor2))=0 then
  lcColor2 = color_code
endif
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+60)+'#YT108/2///'+'PO: '+alltrim(po_num)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T150.0#J'+str(lnOffset+60)+'#YT108/2///'+'DEPT: '+alltrim(department)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+65)+'#YT108/2///'+'STYLE: '+alltrim(style)+' '+alltrim(lcColor2)+ ;
	' '+alltrim(str(lnTotalBlockQty))+' UNITS'+'#G'
if isPrepack then
	lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+70)+'#YT108/2///'+pack_description+'#G'
else
	lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+70)+'#YT108/2///'+casedescription+'#G'
endif
...
lcStringBlock3=lcStringBlock3+'#T95.0#J'+str(lnOffset+8)+'#YT106/2///CARTON '+ ;
	'______ of ______     WEIGHT: _______#G'
&&	alltrim(str(liIterationCounter))+' of '+alltrim(str(liCountCurGroupByCarton))+'     WEIGHT: _______#G'
lcStringBlock3=lcStringBlock3+'#T95.0#J'+str(lnOffset+12)+'#YT106/2///CTN DIM: ____L ____W ____H COUNTRY ORIGIN: _______#G'
...
ENDIF
lcStringBlock3 = lcStringBlock3 + '#T95.0#J'+str(lnOffset+22)+'#YT105/2///'+'FOR:'+'#G'
lcStringBlock3 = lcStringBlock3 + '#M2/2#T80.0#J'+str(lnOffset+22)+'#YT108/2///##'+ALLTRIM(store)+' '+ ;
    alltrim(sfsstore_name)+'#G#M1/1'
lcStringBlock3 = lcStringBlock3 + '#T95.0#J'+str(lnOffset+27)+'#YT105/2///'+'PROD:'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T80.0#J'+str(lnOffset+27)+'#YT108/2///'+alltrim(sfsdept_name)+'#G'
