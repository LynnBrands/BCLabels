...
SCAN
	...
	if MOD(lnRecordPointCurPart2,2*MaxNbLineBlock2)<=MaxNbLineBlock2 AND MOD(lnRecordPointCurPart2,2*MaxNbLineBlock2)>0
		*- Insert a new record in CurPart2Final
		...
		replace color_code_col2 with lcColor2 in CurPart2Final
	...
	*- second column header
	...
	DO WHILE !eof('curPart2Final') AND lnIndLineBlock2<=MaxNbLineBlock2
		...
		*- Color
		...
		if LEN(ALLTRIM(lcColor))=0 THEN 
			lcColor=sfscolor_name 
		ENDIF
...
*- Zone C/D (Code 39 Extended barcode)

liCartonCount = vl_pickcartoncount(pick_num)
lcBPBarcode = ALLTRIM(po_num)+'X1X'+alltrim(str(carton_seq))
lcStringBlock3 = lcStringBlock3 + '#T195.0#J'+str(lnOffset+55)+'#YB23/2M/3/3///'+lcBPBarcode+'#G'
...
*-----------------------------------------------------------------------------
*- FUNCTION vl_pickcartoncount(pick_num)
*- Returns the carton count from the pick header
*- Michael J. Sabal, Notations, Inc. 27 Feb 2017
*-----------------------------------------------------------------------------
FUNCTION vl_pickcartoncount
PARAMETERS parm_picknum

  LOCAL lcSQLQuery, llCount, lcSQLReturn

  lcSQLQuery = "SELECT carton from zzoordrh "+;
	"WHERE pick_num="+alltrim(str(parm_picknum))
  = v_SQLPrep(lcSQLQuery,'rc','')
  if used('rc') and reccount('rc')>0
    *- We have valid notes
	lcSQLReturn = rc.carton
  else
    lcSQLReturn = sfsmst_qty
  endif
