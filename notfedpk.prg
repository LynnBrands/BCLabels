...
*- PART 3: UCC128 part of the label - FEDERATED CORP. (05/12/2005)
*- ----------------------------------------------------------------------------------------------------------------------- -*
	...
	*lcStringBlock1=lcStringBlock1+SPACE(5)
	...
	DO WHILE !eof('curPart1') AND lnIndLineBlock1<=MaxNbLineBlock1
		...
		*- Label
		...
		lcCartonSeq = alltrim(str(carton_seq))
...
if isPrepack then
	lcStringBlock1 = lcStringBlock1+'#M2/2#T100.0#J'+str(lnOffset+57)+'#YT109/2M///PREPACK#G#M1/1'
endif 
lcStringBlock1=lcStringBlock1+'#T200.0#J'+str(lnOffset+25+(lnIndLineBlock1*4))+'#YT106/2///'+ ;
	'Ship To: '+ alltrim(lcShipTo) + '#G'

&& Macys In-DC initiative, effective 1 February 2011.
&& Change request: http://vo.notations.com/?q=node/3294
&& 5 and 4 day transits are group 1, 3-2-1 day transits are group 2
if allt(lcLocation) = 'WPAFG' then
	lcMacyGroup = "0"
	if allt(lcDC) = 'CI' OR allt(lcDC) = 'HA' or ;
		allt(lcDC) = 'VF' or allt(lcDC) = 'MR' or ;
		allt(lcDC) = 'TU' then
		lcMacyGroup = "1"
	endif
	if allt(lcDC) = 'AZ' or allt(lcDC) = 'DV' or allt(lcDC) = 'HU' then
		lcMacyGroup = "1"
	endif
	if allt(lcDC) = 'BI' or ;
		allt(lcDC) = 'CL' or allt(lcDC) = 'GN' or ;
		allt(lcDC) = 'PD' or allt(lcDC) = 'ST' or ;
		allt(lcDC) = 'BA' then
		lcMacyGroup = "2"
	endif
	if allt(lcDC) = 'JP' or allt(lcDC) = 'SW' or ;
		allt(lcDC) = 'WJ' or allt(lcDC) = 'MB' then 
		lcMacyGroup = "2"
	endif
	if allt(lcDC) = 'SC' then
		lcMacyGroup = "2"
	endif
	lcStringBlock1 = lcStringBlock1 + '#M2/2#T100.0#J'+str(lnOffset+70)+'#YT109/2///'+ ;
		lcMacyGroup+'#G#M1/1'
else if allt(lcLocation) = 'WMELKO' then
	lcMacyGroup = "0"
	if allt(lcDC) = 'CI' OR allt(lcDC) = 'MR' then
		lcMacyGroup = "2"
	endif
	if allt(lcDC) = 'AZ' or allt(lcDC) = 'HA' then
		lcMacyGroup = "2"
	endif
	if allt(lcDC) = 'DV' or allt(lcDC) = 'HU' or ;
		allt(lcDC) = 'VF' or allt(lcDC) = 'TU' then
		lcMacyGroup = "2"
	endif
	if allt(lcDC) = 'BI' or allt(lcDC) = 'CL' or ;
		allt(lcDC) = 'BA' then
		lcMacyGroup = "1"
	endif
	if allt(lcDC) = 'SC' or allt(lcDC) = 'GN' or ;
		allt(lcDC) = 'PD' or allt(lcDC) = 'ST' or ;
		allt(lcDC) = 'JP' or allt(lcDC) = 'SW' or ;
		allt(lcDC) = 'WJ' then
		lcMacyGroup = "1"
	endif
	lcStringBlock1 = lcStringBlock1 + '#M2/2#T100.0#J'+str(lnOffset+70)+'#YT109/2///'+ ;
...
&& End of In-DC
lcStringBlock1 = lcStringBlock1 + '#T60.0#J'+str(lnOffset+70)+'#YT107/2///Carton Seq: '+ ;
	lcCartonSeq+'#G'
...
*- MACYS CORP.
*----------------------------------------------------------------------------------------------------
*- Changes effective January 2017
*- 1) Print "REPLENISHMENT" in Zone G 20point for QR (BEG02=RE)
*- 2) Move FOBName to Zone G below "REPLENISHMENT"
*- 3) Store address should be printed in Zone H
*----------------------------------------------------------------------------------------------------
...
&& Prior to Jan 2017, divider was at 86mm
...
ENDIF
IF LEN(B_ADDR1)>25 THEN
  B_ADDR1 = SUBSTR(B_ADDR1,1,RAT(' ',B_ADDR1))
ENDIF
B_NAME1 = ALLTRIM(B_NAME)
B_NAME2 = ""
IF LEN(B_NAME1)>25 THEN
	B_NAME2 = SUBSTR(B_NAME1,RAT(' ',B_NAME1,2),LEN(B_NAME1))
	B_NAME1 = SUBSTR(B_NAME1,1,RAT(' ',B_NAME1,2))
ENDIF
lcStringBlock3 = lcStringBlock3 + '#T158.0#J'+str(lnOffset+2)+'#YT106/2///'+'TO:'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T155.0#J'+str(lnOffset+9)+'#YT106/2///'+B_NAME1+'#G'
lcStringBlock3 = lcStringBlock3 + '#T155.0#J'+str(lnOffset+12)+'#YT106/2///'+B_NAME2+'#G'
...
*- Zone D (Carrier, etc.)
lcStringBlock3 = lcStringBlock3 + '#T138.0#J'+str(lnOffset+35)+'#YT102/2///Carrier:#G'
lcStringBlock3 = lcStringBlock3 + '#T138.0#J'+str(lnOffset+45)+'#YT102/2///BOL:#G'
...
lcBEG02 = vl_beg02(ord_num,pick_num)
if alltrim(lcBEG02)='RE' then
  lcStringBlock3 = lcStringBlock3 + '#T100.0#J'+STR(lnOffset+12)+'#YT109/2///REPLENISHMENT#G'
endif
  && Updated 10/7/2008 Michael Sabal to use po_desc as the FOB name when available
lcFOB = vl_fobname(ord_num,pick_num)
if (LEN(alltrim(lcFOB))=0) then
  lcFOB=sfsdept_name
endif
lcStringBlock3 = lcStringBlock3 + '#T100.0#J'+STR(lnOffset+24)+'#YT109/2///'+ALLTRIM(lcFOB)+'#G'


*- Zone H (Mark for store # [text])
&& Prior to Jan 2017, the divider between G and H was at 86mm, with zone H text left justified at 84mm.
...
ENDIF
lcStringBlock3 = lcStringBlock3 + '#T49.0#J'+str(lnOffset+3)+'#YT102/2///'+'FOR:'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T48.0#J'+str(lnOffset+8)+'#YT108/2///'+alltrim(sfscust_name)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T05.0#J'+str(lnOffset+12)+'#M1/2#YT108/2R///ST##'+ALLTRIM(store)+'#G#M1/1'
&& lcStringBlock3 = lcStringBlock3 + '#T48.0#J'+STR(lnOffset+10)+'#YT108/2///ST##'+ALLTRIM(store)+'#G'
&& Prior to Jan 2017, font for store name was YT108
S_NAME1 = ALLTRIM(sfsstore_name)
S_NAME2 = ""
IF LEN(S_NAME1)>25 THEN
	S_NAME2 = SUBSTR(S_NAME1,RAT(' ',S_NAME1,2),LEN(S_NAME1))
	S_NAME1 = SUBSTR(S_NAME1,1,RAT(' ',S_NAME1,2))
ENDIF
lcStringBlock3 = lcStringBlock3 + '#T48.0#J'+STR(lnOffset+16)+'#YT106/2///'+S_NAME1+'#G'
lcStringBlock3 = lcStringBlock3 + '#T48.0#J'+STR(lnOffset+19)+'#YT106/2///'+S_NAME2+'#G'
S_ADDR1 = alltrim(sfsmark_addr1)
S_ADDR2 = alltrim(sfsmark_addr2)
S_CSZ = alltrim(sfsmark_city)+', '+alltrim(sfsmark_state)+'  '+alltrim(sfsmark_zip)
lcStringBlock3 = lcStringBlock3 + '#T48.0#J'+STR(lnOffset+22)+'#YT106/2///'+S_ADDR1+'#G'
lcStringBlock3 = lcStringBlock3 + '#T48.0#J'+STR(lnOffset+25)+'#YT106/2///'+S_ADDR2+'#G'
lcStringBlock3 = lcStringBlock3 + '#T48.0#J'+STR(lnOffset+28)+'#YT106/2///'+S_CSZ+'#G'
...
*-----------------------------------------------------------------------------
*- FUNCTION vl_beg02(ord_num, pick_num)
*- Returns Range Style from zzoordrd
*- Michael J. Sabal, Notations, Inc. 15 June 2007
*-----------------------------------------------------------------------------
...
PARAMETERS parm_ordnum, parm_picknum
  ...
  = v_SQLPrep(lcSQLQuery,'beg','')
  if used('beg') and reccount('beg')>0
    *- Success
    lcSQLReturn = beg.EDI_POTYPE
  else
    lcSQLReturn = ""
  endif
