		...
		*- Label
		...
		if isPrepack then
			...
			*- Total
			...
			lcSKU_UPC = vl_cstdr(customer,'CUST_STYLE','',division,Style,color_code,lbl_code,dimension,1)
			IF EMPTY(lcSKU_UPC)
				liBadSku = 1
			ENDIF
		else
			*- Bucket quantities
			...
			for lnIndbucket=2 to 10
				...
				*if eval('curPart1.siz'+lcIndbucket+'_qty')>0 and lnIndValidBucket<10
					...
					lnIndValidBucket=lnIndValidBucket+1
					lcSKU_UPC = vl_cstdr(customer,'CUST_STYLE','',division,Style,color_code,lbl_code,dimension,lnIndbucket)
					IF EMPTY(lcSKU_UPC)
						liBadSku = 1
					ENDIF
				*endif
...
if alltrim(customer)="SHOPKO" and LEN(alltrim(lcSKU_UPC))>0 then
  lcStringBlock3 = lcStringBlock3 + '#T150.0#J'+str(lnOffset+65)+'#YT108/2///'+'SKU: '+lcSKU_UPC+'#G'
endif
