...
*- this Foxpro program NOTSEABK.PRG (orig: NOTPCLUC.PRG)
...
*- PART 3: UCC128 part of the label - Bulk Order version of Sears Holding Corp. (01/21/2008)
*- ----------------------------------------------------------------------------------------------------------------------- -*
...
scan
	...
	*- -----------------------------------------------------------------------------------------------
	...
	lcLblMask = cur2Temp.lbl_code
	if (alltrim(cur2Temp.Style)=='JITF0549' and alltrim(cur2Temp.Color_code)=='ERIE') OR ;
	   (alltrim(cur2Temp.Style)=='JNSU0126' and (alltrim(cur2Temp.Color_code)=='IVORY' OR ;
		alltrim(cur2Temp.Color_code)=='BLACK')) OR ;
	   (alltrim(cur2Temp.Style)=='JNSU0127' and (alltrim(cur2Temp.Color_code)=='WHITE' OR ;
		alltrim(cur2Temp.Color_code)=='BLACK')) 
		lcLblMask = ''
	endif
	liRangeBucket = 2
	if (alltrim(cur2Temp.Style)=='MQRP0101') then
		liRangeBucket = 3
	endif
	loRangeStyle = vl_rangestyle(cur2Temp.ord_num,cur2Temp.pick_num,cur2Temp.Style,cur2Temp.Color_code,cur2Temp.lbl_code)
        lcGTIN11 = vl_upcsr(cur2Temp.Division,'upc_num','',loRangeStyle.rng_style,loRangeStyle.rng_color,;
			loRangeStyle.rng_lbl,cur2Temp.Dimension,liRangeBucket)
        lcGTIN13 = "00"+lcGTIN11
        lcGTIN14 = lcGTIN13 + gtin14_mod10(lcGTIN13)
...
*- Sears, Inc. - non-JIT, Bulk Orders
*- Zones A-D across the top.
*- SSCC-18 in zones E/F on lower left
*- Zones G/H will be absent
*- GTIN-14 in zone I on lower right

*- Sears rules for creating the GTIN-14, from John Verb at BarCode Graphics.
*- Corrected by MikeS 26 Mar 2009
*- If using an exploding assortment, the GTIN = "00" + prepack UPC.
*- If shipping single size per carton, the GTIN = "10" + item UPC.
...
lcStringBlock3 = lcStringBlock3 + '#T40.1#J'+str(lnOffset+0)+'#YL0/1/1/30#G'
...
lcStringBlock3 = lcStringBlock3 + '#T88.0#J'+str(lnOffset+25)+'#YB15/2O/3.5/5///420'+alltrim(sfsship_zip)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T78.0#J'+str(lnOffset+29)+'#YT107/2///(420)'+alltrim(sfsship_zip)+'#G'

*- Zone D (BOL & PO#)

lcStringBlock3 = lcStringBlock3 + '#T37.0#J'+str(lnOffset+10)+'#YT107/2///'+'P.O. NUMBER: '+'#G'
lcStringBlock3 = lcStringBlock3 + '#T37.0#J'+str(lnOffset+17)+'#YT108/2///'+substr(po_num,1,5)+ ;
    '-'+alltrim(substr(po_num,6))+'#G'

*- Zone E/F (SSCC-18 on left)

lcStringBlock3 = lcStringBlock3 + '#T193.0#J'+str(lnOffset+64)+'#YB15/2O/5.5/5///'+alltrim(carton_num)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T193.0#J'+str(lnOffset+69)+'#YT108/2///('+substr(carton_num,1,2)+ ;
  ') '+substr(carton_num,3,1)+' '+substr(carton_num,4,7)+' '+substr(carton_num,11,9)+' '+ ;
  substr(carton_num,20,1)+'#G'

*- Zone G/H (Not used by Sears non-JIT)

*- Zone I (GTIN-14 on right)

&& Interleaved 2 of 5
lcStringBlock3 = lcStringBlock3 + '#T91.0#J'+str(lnOffset+64)+'#YB12/2O/5/7///'+alltrim(lcGTIN14)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T80.0#J'+str(lnOffset+69)+'#YT108/2///'+substr(lcGTIN14,1,1)+' '+ ;
  substr(lcGTIN14,2,2)+' '+substr(lcGTIN14,4,5)+' '+substr(lcGTIN14,9,5)+' '+ ;
  substr(lcGTIN14,14,1)+'#G'
...
PARAMETERS parm_ordnum, parm_picknum, parm_style, parm_color, parm_label
  ...
  lcSQLQuery = "SELECT rng_style,rng_color,rng_lbl FROM zzoordrd WHERE ord_num="+alltrim(str(parm_ordnum))+;
  ...
  if used('RangeStyle') and reccount('RangeStyle')>0
    loObjReturn = createobject('Custom')
	with loObjReturn
		.AddProperty("rng_style",SPACE(0))
		.AddProperty("rng_color",SPACE(0))
		.AddProperty("rng_lbl",SPACE(0))
		.rng_style = RangeStyle.rng_style
		.rng_color = RangeStyle.rng_color
		.rng_lbl = RangeStyle.rng_lbl
	endwith
    *- We have a range style
    lcSQLReturn = loObjReturn
  else
    lcSQLReturn = ""
  endif
