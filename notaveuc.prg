...
*- this Foxpro program NOTMAYUC.PRG (orig: NOTPCLUC.PRG)
...
*- PART 3: UCC128 part of the label - MAY CORP. (04/01/05)
*- ----------------------------------------------------------------------------------------------------------------------- -*
		...
		IF curPart1.size_code<>lcCurSize_code and not isPrepack then
			...
			*- Maximum 9 size headers
			...
			for lnIndbucket=2 to 12
				...
				*if eval('curPart1.siz'+lcIndbucket+'_qty')>0 and lnIndValidBucket<10
					lcStringBlock1=lcStringBlock1+ ;
						'#T'+str(173.3-(lnIndbucket*10.0))+'#J'+str(lnOffset+21+(lnIndLineBlock1*4))+'#YT102/2///'+ ;
						padl(eval('curPart1.siz'+lcIndbucket+'_head'),6)+'#G'
					lnIndValidBucket=lnIndValidBucket+1
				*endif
			endfor
		...
		if isPrepack then
			...
			*- Bucket quantities
			...
			for lnIndbucket=2 to 12
				lcIndbucket=TRANS(lnIndbucket,"@L 99")
				*if eval('curPart1.siz'+lcIndbucket+'_qty')>0 and lnIndValidBucket<10
					lcStringBlock1=lcStringBlock1+ '#T'+str(173.3-(lnIndbucket*10.0))+'#J'+ ;
						str(lnOffset+21+(lnIndLineBlock1*4))+'#YT103/2///'+ ;
						padl(allt(str(eval('curPart1.siz'+lcIndbucket+'_qty'))),6)+"#G"
...
SCAN
	...
	lcColor = cust_color(CurPart2.customer,CurPart2.style,CurPart2.color_code)
ENDSCAN
...
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+40)+'#YT108/2///'+lcCartonXofY+'#G'
lcStringBlock3=lcStringBlock3+'#T200.0#J'+str(lnOffset+45)+'#YT102/2///'+ ;
	'PRETICKETED: YES'+'#G'
...
if lnSKUCount = 1 and not isPrepack then    && Bulk pack
  lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+64)+'#YT104/2///'+'STYLE: '+alltrim(lcSKU_UPC)+'#G'
  lcStringBlock3 = lcStringBlock3 + '#T167.0#J'+str(lnOffset+64)+'#YT104/2///'+'COLOR: '+alltrim(lcColor)+'#G'
...
ELSE 
  lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+60)+'#YT104/2///'+'STYLE: '+alltrim(lcSKU_UPC)+'#G'
  lcStringBlock3 = lcStringBlock3 + '#T167.0#J'+str(lnOffset+60)+'#YT104/2///'+'COLOR: '+alltrim(lcColor)+'#G'
...
ENDIF
lcStoreName = sfsstore_name
lcDeptName = sfsdept_name
IF alltrim(lbl_code)="FAVD" OR alltrim(lbl_code)="FVIE" OR alltrim(lbl_code)="FLOD" THEN
	lcStoreName = STRTRAN(sfsstore_name,"AVENUE","AVENUE.COM")
	lcDeptName = STRTRAN(sfsdept_name,"AVENUE","AVENUE.COM")
ENDIF
lcStringBlock3 = lcStringBlock3 + '#M2/2#T98.0#J'+str(lnOffset+12)+'#YT108/2///##'+ALLTRIM(store)+'#G#T98.0#J'+ ;
    str(lnOffset+24)+"#YT108/2///"+alltrim(lcStoreName)+'#G#M1/1'
