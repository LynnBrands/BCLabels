...
DO WHILE NOT EOF('CurPart2')
  ...
  *- Detail header
  ...
  lcPrintString = lcPrintString + "^FO80,325^A0N,25,20^FDUPC/SKU^FS"+;
  	"^FO350,325^A0N,25,20^FDSTYLE^FS"+;
	"^FO550,325^A0N,25,20^FDCOLOR^FS"+;
	"^FO650,325^A0N,25,20^FDSIZE^FS"+;
	"^FO725,325^A0N,25,20^FDQUANTITY^FS"
  lnOffset=360
  lnLineNum=0
  DO WHILE NOT EOF('CurPart2') and lnLineNum<30
    lcPrintString = lcPrintString + "^FO80,"+allt(str(lnOffset))+"^ACN,22,10^FD"+;
	padr(SKU_UPC,27)+"^FS"+;
	"^FO350,"+ALLTRIM(STR(lnOffset))+"^ACN,22,10^FD"+style+"^FS"+;
	"^FO550,"+ALLTRIM(STR(lnOffset))+"^ACN,22,10^FD"+color_code+"^FS"+;
	"^FO650,"+ALLTRIM(STR(lnOffset))+"^ACN,22,10^FD"+size_desc+"^FS"+;
	"^FO725,"+ALLTRIM(STR(lnOffset))+"^ACN,22,10^FD"+PADL(ALLTRIM(str(Total_qty)),6)+"^FS"
