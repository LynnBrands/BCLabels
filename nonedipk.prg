...
mixedCarton = (reccount('curPart1')>1)
			...
			if used('prepack') and reccount('prepack')>0 then
				...
				for lnIndbucket=2 to 10
					...
					if eval('curPart1.siz'+lcIndbucket+'_qty')>0 then
						lcMakePack = lcMakePack + alltrim(eval('curPart1.siz'+lcIndbucket+'_head'))+"/"+alltrim(str(eval('curPart1.siz'+lcIndbucket+'_qty')))+" "
						pack_qty = pack_qty + eval('curPart1.siz'+lcIndbucket+'_qty')
					endif
				endfor
				lcMakePack = lcMakePack + "= "+alltrim(str(pack_qty,6))
				pack_description = lcMakePack
			endif
			lnPrepack_qty=eval('curPart1.siz01_qty')
			if lnPrepack_qty = 0 then
				lnPrepack_qty = pack_qty
			endif
...
lcSKU_UPC = vl_cstdr(customer,'CUST_STYLE','',division,Style,color_code,lbl_code,dimension,lnLastSize)

IF EMPTY(lcSKU_UPC) AND lnSKUCount = 1 
	*- zveupcnr.upc_num = char(11)
	lcUPC=vl_upcsr(Division,'upc_num','',Style,Color_code,lbl_code,Dimension,lnLastSize)
	lcUPC_chk=vl_upcsr(Division,'chk_digit','',Style,Color_code,lbl_code,Dimension,lnLastSize)		
...
lcStringBlock3 = lcStringBlock3 + '#T203.2#J'+str(lnOffset+50)+'#YL0/2/1/101.6#G'
lcStringBlock3 = lcStringBlock3 + '#T159.3#J'+str(lnOffset+0)+'#YL0/1/1/25#G'
lcStringBlock3 = lcStringBlock3 + '#T140.1#J'+str(lnOffset+25)+'#YL0/1/1/25#G'
...
IF len(ALLTRIM(sfsship_zip))>5
  lcStringBlock3 = lcStringBlock3 + '#T188.0#J'+str(lnOffset+45)+'#YB15/2O/3/5///420'+substr(sfsship_zip,1,5)+'#G'
  lcStringBlock3 = lcStringBlock3 + '#T178.0#J'+str(lnOffset+49)+'#YT107/2///(420)'+substr(sfsship_zip,1,5)+'#G'
ELSE
  lcStringBlock3 = lcStringBlock3 + '#T188.0#J'+str(lnOffset+45)+'#YB15/2O/3/5///420'+alltrim(sfsship_zip)+'#G'
  lcStringBlock3 = lcStringBlock3 + '#T178.0#J'+str(lnOffset+49)+'#YT107/2///(420)'+alltrim(sfsship_zip)+'#G'
ENDIF

*- Zone D (Carrier, etc.)
lcCartonXofY = vl_cartonudf3(carton_num)
lcStringBlock3=lcStringBlock3+'#T138.0#J'+str(lnOffset+40)+'#YT103/2///'+lcCartonXofY+'#G'


*- Zone E (PO / Dept)

lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+56)+'#YT104/2///'+'PO: '+alltrim(po_num)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T167.0#J'+str(lnOffset+56)+'#YT104/2///'+'DEPT: '+alltrim(department)+'#G'
if not mixedCarton
	if lnSKUCount = 1 and not isPrepack then    && Bulk pack
	  lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+64)+'#YT104/2///'+'STYLE: '+alltrim(style)+'#G'
	  lcStringBlock3 = lcStringBlock3 + '#T150.0#J'+str(lnOffset+64)+'#YT104/2///'+'COLOR: '+alltrim(color_code)+'#G'
	  *- lcStringBlock3 = lcStringBlock3 + '#T140.0#J'+str(lnOffset+64)+'#YT104/2///'+'SIZE: '+alltrim(size_desc)+'#G'
	  lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+70)+'#YT104/2///'+'SIZE: '+alltrim(size_desc)+'#G'
	  lcStringBlock3 = lcStringBlock3 + '#T135.0#J'+str(lnOffset+56)+'#YT104/2///'+'QTY: '+alltrim(str(lnTotalBlockQty))+'#G'
	  lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+60)+'#YT104/2///'+'ITEM: '+alltrim(lcSKU_UPC)+'#G'
	ELSE 
	  lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+60)+'#YT104/2///'+'STYLE: '+alltrim(style)+'#G'
	  lcStringBlock3 = lcStringBlock3 + '#T150.0#J'+str(lnOffset+60)+'#YT104/2///'+'COLOR: '+alltrim(color_code)+'#G'
	  lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+64)+'#YT104/2///SIZE#G'
	  lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+68)+'#YT104/2///QTY ('+alltrim(str(lnTotalBlockQty))+')#G'
	  select curPart1
	  go top
		for lnIndbucket=2 to 9
			...
			lcStringBlock3=lcStringBlock3+ ;
				...
				padl(eval('curPart1.siz'+lcIndbucket+'_head'),6)+'#G'
		endfor
		if isPrepack then
			getPPKExplosion(division,size_code,dimension)
			lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+72)+'#YT104/2///'+'ITEM: '+alltrim(lcSKU_UPC)+'#G'
			lnPackCnt = lnTotalBlockQty/explarr[1]
			if explarr[1]>-1 then
				for lnIndbucket=2 to 9
					if explarr[lnIndbucket] > 0 then
						lcIndbucket=TRANS(lnIndbucket,"@L 99")
						lcStringBlock3=lcStringBlock3+ ;
							'#T'+str(180-((lnIndbucket-2)*10))+'#J'+str(lnOffset+68)+'#YT104/2///'+ ;
							padl(alltrim(str(lnPackCnt * explarr[lnIndbucket])),6)+'#G'
					endif
				endfor
...
*---- 11 Feb 2019: Ric no longer wishes to support size explosion packs, in the event a malformed code is included on a detail line in error.
		else
*-			getSizeExplosion(pick_num,line_seq)
			for lnIndbucket=2 to 9
				if eval('curPart1.siz'+lcIndbucket+'_qty') > 0 then
					...
					lcStringBlock3=lcStringBlock3+ ;
						'#T'+str(180-((lnIndbucket-2)*10))+'#J'+str(lnOffset+68)+'#YT104/2///'+ ;
						padl(eval('curPart1.siz'+lcIndbucket+'_qty'),6)+'#G'
				endif
			endfor
		endif
	endif
...
*-----------------------------------------------------------------------------
*- FUNCTION vl_cartonudf3(carton_num)
*- Returns Carton X of Y from zzoctnph
*- Michael J. Sabal, Notations, Inc. 18 Sept 2018
*-----------------------------------------------------------------------------

FUNCTION vl_cartonudf3
PARAMETERS parm_carton_num

  LOCAL lcSQLQuery, llCount, lcSQLReturn

  lcSQLQuery = "SELECT carton_udf3 FROM zzoctnph WHERE carton_num='"+alltrim(parm_carton_num)+"'"

  = v_SQLPrep(lcSQLQuery,'beg','')
  if used('beg') and reccount('beg')>0
    *- Success
    lcSQLReturn = beg.carton_udf3
  else
    lcSQLReturn = "CARTON ________ OF ________"
  endif
...
*-----------------------------------------------------------------------------
*- FUNCTION getSizeExplosionPPK(division,size_code,dimension)
*-----------------------------------------------------------------------------
