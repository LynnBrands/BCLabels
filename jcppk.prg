		...
		*- Label
		...
		if isPrepack then
			...
			*- Total
			...
			loRangeStyle = vl_rangestyle(ord_num,pick_num,style,color_code,lbl_code)
			loPackCode = 'UNK PACK'
			if LEN(loRangeStyle)>0 then
				loCSR = vl_cstdr(customer,'CUST_STYLE','',division,loRangeStyle,color_code,lbl_code,'',2)
				if len(loCSR)>8 then
					loPackCode = substr(loCSR,5,4)
				endif
			endif
			*- Bucket quantities
			lnTotal_qty=0
			...
			*- Total
			...
			lcStringBlock1 = lcStringBlock1 + '#M2/2#T100.0#J'+str(lnOffset+70)+'#YT109/2M///'+loPackCode+'#G#M1/1'
		endif
...
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+6)+'#YT106/2///'+'LYNN BRANDS, LLC'+'#G'
...
ENDIF
if len(lc_dcnum)=0 then
  lc_dcnum = alltrim(store)
endif
lcStringBlock3 = lcStringBlock3 + '#T168.0#J'+str(lnOffset+4)+'#YT107/2///'+'TO:'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T165.0#J'+str(lnOffset+9)+'#YT107/2///'+alltrim(sfsship_name)+' ('+lc_dcnum+')#G'
