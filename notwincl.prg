...
*- this Foxpro program NOTWINCL.PRG (orig: NOTPCLUC.PRG)
...
*- PART 3: Long side label (no barcodes). (04/01/05)
*- PART 4: Short side label (no barcodes).
*- ----------------------------------------------------------------------------------------------------------------------- -*
...
??? "#!A1#IMS203.2/317.0#ERN1"   && 8" wide by 9" long "Page"
...
scan
	...
	*- -----------------------------------------------------------------------------------------------
	lcItemDescription = item_description(cur2Temp.customer,cur2Temp.Style,cur2Temp.color_code)
	lcItemNumber = item_number(cur2Temp.customer,cur2Temp.Style,cur2Temp.color_code)
    && lcItemDescription = vl_csthr(cur2Temp.customer,'CUST_DESC','',cur2Temp.division,cur2Temp.Style,cur2Temp.color_code,cur2Temp.lbl_code,cur2Temp.dimension)
    && lcItemNumber = vl_csthr(cur2Temp.customer,'CUSTOMER_STYLE','',cur2Temp.division,cur2Temp.Style,cur2Temp.color_code,cur2Temp.lbl_code,cur2Temp.dimension)
...
lcStringBlock2=lcStringBlock2+'#T33.3#J'+str(lnOffset+17+(lnIndLineBlock2*3))+'#YT106/2///'+ ;
	'Total: '+str(lnTotalBlockQty)+'#G'
lcStringBlock2=lcStringBlock2+'#T200.0#J'+str(lnOffset+25+(lnIndLineBlock2*3))+'#YT102/2///'+ ;
...
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset + 15)+'#M2/2#YT108/2///'+lcItemNumber+'#G#M1/1'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset + 25)+'#M2/2#YT108/2///'+lcSKU_UPC+'#G#M1/1'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset + 35)+'#M2/2#YT108/2///'+lcItemDescription+'#G#M1/1'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset + 45)+'#M2/2#YT108/2///PO '+allt(po_num)+'#G#M1/1'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset + 55)+'#M2/2#YT108/2///CONTENTS '+allt(str(lnTotalBlockQty))+' PCS#G#M1/1'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset + 65)+'#M2/2#YT108/2///______ of _______ CARTONS#G#M1/1'
...
lcCountryOfOrigin = '_________________'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset + 15)+'#M2/2#YT108/2///WMG#G#M1/1'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset + 25)+'#M2/2#YT108/2///CHICAGO#G#M1/1'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset + 35)+'#M2/2#YT108/2///MADE IN '+lcCountryOfOrigin+'#G#M1/1'
...
*-----------------------------------------------------------------------------
*- FUNCTION item_description(customer,style,color)
...
PARAMETERS parm_customer, parm_style, parm_color
  ...
  lcSQLQuery = "SELECT CUST_DESC FROM zzxcsthr WHERE customer='"+ ;
  ...
  if used('CustColor') and reccount('CustColor')>0
    *- We have a valid customer style x-ref
    lcSQLReturn = CustColor.CUST_DESC
  else
...
*-----------------------------------------------------------------------------
*- FUNCTION item_number(customer,style,color)
*- Returns the region code from zzxcslsr
*- Michael J. Sabal, Notations, Inc. 14 Aug 2007
*-----------------------------------------------------------------------------
...
PARAMETERS parm_customer, parm_style, parm_color
  ...
  lcSQLQuery = "SELECT CUSTOMER_STYLE FROM zzxcsthr WHERE customer='"+ ;
               parm_customer+"' AND style='"+parm_style+"' AND "+ ;
               "color_code='"+parm_color+"'"
  = v_SQLPrep(lcSQLQuery,'CustColor','')
  if used('CustColor') and reccount('CustColor')>0
    *- We have a valid customer style x-ref
    lcSQLReturn = CustColor.CUSTOMER_STYLE
  else
    lcSQLReturn = parm_color
  endif
