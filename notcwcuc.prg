...
*- this Foxpro program NOTCWCUC.PRG (orig: NOTPCLUC.PRG)
...
*- PART 3: UCC128 part of the label - COLDWATER CREEK (30 June 2005)
*- ----------------------------------------------------------------------------------------------------------------------- -*
...
scan
	...
	*- -----------------------------------------------------------------------------------------------
    ...
    lcSKU_descr = vl_cstyr(cur2Temp.customer,'CUST_DESC','',cur2Temp.division,cur2Temp.Style,cur2Temp.color_code,cur2Temp.lbl_code,cur2Temp.dimension)
    lcSKU_color = vl_cstyr(cur2Temp.customer,'CUST_COLOR','',cur2Temp.division,cur2Temp.Style,cur2Temp.color_code,cur2Temp.lbl_code,cur2Temp.dimension)
    lcSKU = lcSKU_UPC
	...
	IF EMPTY(lcSKU_UPC)
		*- zveupcnr.upc_num = char(11)
		...
		if len(lcSKU_UPC)>12 then
		  lcSKU_UPC = substr(lcSKU_UPC,1,12)
		endif
	ENDIF
...
DO WHILE NOT EOF('CurPart2Final') 
	...
	*lcStringBlock2=lcStringBlock2+SPACE(5)
	...
	lcStringBlock2=lcStringBlock2+'#T173.3#J'+str(lnOffset+14)+'#YT102/2///'+ ;
		'Size'+'#G'
	lcStringBlock2=lcStringBlock2+'#T165.0#J'+str(lnOffset+14)+'#YT102/2///'+ ;
		'SKU/UPC'+'#G'
	...
	*- second column header
	...
	lcStringBlock2=lcStringBlock2+'#T73.3#J'+str(lnOffset+14)+'#YT102/2///'+ ;
		'Size'+'#G'
	lcStringBlock2=lcStringBlock2+'#T65.0#J'+str(lnOffset+14)+'#YT102/2///'+ ;
		'SKU/UPC'+'#G'
	...
	DO WHILE !eof('curPart2Final') AND lnIndLineBlock2<=MaxNbLineBlock2
		...
		*- SKU/UPC
		lcStringBlock2=lcStringBlock2+'#T165.0#J'+str(lnOffset+17+(lnIndLineBlock2*3))+'#YT102/2///'+ ;
			allt(SKU_UPC)+'#G'
		...
		*- SKU/UPC - Column 2
		lcStringBlock2=lcStringBlock2+'#T65.0#J'+str(lnOffset+17+(lnIndLineBlock2*3))+'#YT102/2///'+ ;
			allt(SKU_UPC_col2)+'#G'
...
lcStringBlock3 = lcStringBlock3 + '#T203.2#J'+str(lnOffset+44)+'#YL0/2/1/101.6#G'
lcStringBlock3 = lcStringBlock3 + '#T159.3#J'+str(lnOffset+0)+'#YL0/1/1/22#G'
lcStringBlock3 = lcStringBlock3 + '#T140.1#J'+str(lnOffset+22)+'#YL0/1/1/22#G'
...
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+6)+'#YT102/2///'+'Vendor: N1109'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+9)+'#YT102/2///'+'NOTATIONS, INC.'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+12)+'#YT102/2///'+'539 JACKSONVILLE RD.'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+15)+'#YT102/2///'+'WARMINSTER, PA'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T170.0#J'+str(lnOffset+15)+'#YT102/2///'+'18974'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+18)+'#YT102/2///'+'SO##:'+alltrim(str(ord_num))+'#G'
...
lcStringBlock3 = lcStringBlock3 + '#T188.0#J'+str(lnOffset+39)+'#YB15/2O/2/5///420'+alltrim(sfsship_zip)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T178.0#J'+str(lnOffset+43)+'#YT107/2///(420)'+alltrim(sfsship_zip)+'#G'
...
*- Zone E (PO / Dept)
	...
	** Quantity
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+STR(lnOffset+52)+'#YT102/2///QUANTITY: '+'#G'
lcStringBlock3 = lcStringBlock3 + '#T185.0#J'+STR(lnOffset+52)+'#YT109/2///'+ALLTRIM(STR(lnTotalBlockQty))+'#G'
	** PO #
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+62)+'#YT102/2///'+'PONBR: #G'
lcStringBlock3 = lcStringBlock3 + '#T185.0#J'+STR(lnOffset+62)+'#YT109/2///'+alltrim(po_num)+'#G'
...
lpos = ATC('@',lcSKU,1)
IF lpos > 1 then
  lcSKUCB = SUBSTR(lcSKU,1,lpos-1)
ELSE
  lcSKUCB = ""
ENDIF
IF lpos < LEN(lcSKU) then
  lcSKU = SUBSTR(lcSKU,lpos+1,LEN(lcSKU)-lpos)
ELSE
  lcSKU = ""
ENDIF
lpos = ATC('@',lcSKU,1)
IF lpos > 1 then
  lcSKUST = SUBSTR(lcSKU,1,lpos-1)
ELSE
  lcSKUST = ""
ENDIF
IF lpos < LEN(lcSKU) then
...
ELSE
  lcSKU = ""
ENDIF
*********** Changed from 2 characters to 3 characters per Ric's executive decision 12/2/2005 ****************
IF LEN(lcSKU) > 0 THEN
  IF LEN(lcSKU) > 3 THEN
    lcSKUIZ = SUBSTR(lcSKU,1,3)
  ELSE
    lcSKUIZ = lcSKU
  ENDIF
ELSE
  lcSKUIZ = ""
ENDIF
lcStringBlock3 = lcStringBlock3 + '#T100.0#J'+str(lnOffset+10)+'#YT102/2///'+'ITEMNBR: #G'
lcStringBlock3 = lcStringBlock3 + '#T85.0#J'+STR(lnOffset+10)+'#YT108/2///'+alltrim(lcSKUCB)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T77.0#J'+STR(lnOffset+10)+'#YT109/2///'+alltrim(lcSKUST)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T45.0#J'+STR(lnOffset+10)+'#YT108/2///'+alltrim(lcSKUCL)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T35.0#J'+STR(lnOffset+10)+'#YT109/2///'+alltrim(lcSKUSZ)+'#G'
if empty(lcSKU_descr) then
  lcStringBlock3 = lcStringBlock3 + '#T85.0#J'+str(lnOffset+15)+'#YT104/2///'+ALLTRIM(sfcstylename)+ ;
	' '+ALLTRIM(lcSKUIZ)+' '+'#G'
else
  lcStringBlock3 = lcStringBlock3 + '#T85.0#J'+str(lnOffset+15)+'#YT104/2///'+alltrim(lcSKU_descr)+ ;
	' '+ALLTRIM(lcSKU_color)+' '+ALLTRIM(lcSKUIZ)+' '+'#G'
endif
	** Prepack/multipack
lcStringBlock3 = lcStringBlock3 + '#T100.0#J'+STR(lnOffset+20.0)+'#YT103/2///PREPACK: No#G'	
lcStringBlock3 = lcStringBlock3 + '#T70.0#J'+STR(lnOffset+20.0)+'#YT103/2///MULTI-PACK: No#G'	
	** Ticketed
lcStringBlock3 = lcStringBlock3 + '#T40.0#J'+STR(lnOffset+20.0)+'#YT103/2///TICKETED: Yes#G'	

*- Zone G (Mark for store # [UCC128])

****** Coldwater Creek does not use zones G or H ******

*lcStore = ALLTRIM(store)
*IF LEN(lcStore)<>4 then
*  lcStore = '0'+lcStore
*ENDIF
*lcStringBlock3 = lcStringBlock3 + '#T93.0#J'+STR(lnOffset+25)+'#YB15/2M B/2.5/5///(91)'+ALLTRIM(lcStore)+'#G'
...
ENDIF
*lcStringBlock3 = lcStringBlock3 + '#T50.0#J'+str(lnOffset+5)+'#YT105/2///'+'FOR:'+'#G'
*lcStringBlock3 = lcStringBlock3 + '#T50.0#J'+str(lnOffset+12)+'#YT108/2///'+alltrim(sfsstore_name)+'#G'
*lcStringBlock3 = lcStringBlock3 + '#T50.0#J'+str(lnOffset+17)+'#YT105/2///'+'STORE NUMBER:'+'#G'
*lcStringBlock3 = lcStringBlock3 + '#T50.0#J'+str(lnOffset+24)+'#YT108/2///'+alltrim(store)+'#G'

*- Zone I (Carton # [UCC128])

lcStringBlock3 = lcStringBlock3 + '#T93.0#J'+str(lnOffset+60)+'#YB15/2O/5/6///'+alltrim(carton_num)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T91.0#J'+str(lnOffset+69)+'#YT109/2///'+alltrim(carton_num)+'#G'
