...
*- this Foxpro program NONEDIPS.PRG (orig: NOTPCLUC.PRG)
...
*- PART 3: UCC128 part of the label - NON-EDI CUSTOMERS (5/11/2005)
*- ----------------------------------------------------------------------------------------------------------------------- -*
	...
	for lnIndbucket = 1 to 9
		...
		*- Size header for this bucket = new fields size01_head,..,size22_head
		...
		replace siz&lcIndbucket._head with padc(allt(eval('curSizeHeader.' + lcFieldName)), 7) in CurPart1
	        if (alltrim(lcStyle)='JCFR0001') then
                  replace siz&lcIndbucket._head with padc(allt(str(lnIndbucket-1)),7) in CurPart1
                endif
        endfor	&& End for 9
...
DO WHILE NOT EOF('CurPart2Final') 
	...
	*- second column header
	...
	DO WHILE !eof('curPart2Final') AND lnIndLineBlock2<=MaxNbLineBlock2
          ...
          if (alltrim(style)='JSLU0124') OR (ALLTRIM(style)='JMEU0034') then
            DO CASE
              CASE alltrim(size_desc)='S'
                replace size_desc with '0' in CurPart2Final
              CASE alltrim(size_desc)='M'
                replace size_desc with '1' in CurPart2Final
              CASE alltrim(size_desc)='L'
                replace size_desc with '2' in CurPart2Final
              CASE alltrim(size_desc)='XL'
                replace size_desc with '3' in CurPart2Final
            ENDCASE
          endif		
          if (alltrim(style_col2)='JCFR0001') then
            DO CASE
              CASE alltrim(size_desc_col2)='S'
                replace size_desc_col2 with '0' in CurPart2Final
              CASE alltrim(size_desc_col2)='M'
                replace size_desc_col2 with '1' in CurPart2Final
              CASE alltrim(size_desc_col2)='L'
                replace size_desc_col2 with '2' in CurPart2Final
              CASE alltrim(size_desc_col2)='XL'
                replace size_desc_col2 with '3' in CurPart2Final
            ENDCASE
          endif		
