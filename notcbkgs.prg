...
*- this Foxpro program NOTCBKGS.PRG (orig: NOTPCLUC.PRG)
...
*- PART 2: Christopher & Banks carton content label
*- PART 3: Christopher & Banks GS-1 carton marking label (06/28/10)
*- ----------------------------------------------------------------------------------------------------------------------- -*
...
ORDER BY carton_num,style,color_code,sizebucket
...
&&	lcPackCode = vl_fobname(ord_num,pick_num)
&&	lcPrepack = vl_poline(pick_num,line_seq)
...
&& lcCtnCode = vl_fobname(ord_num,pick_num)
&& lcPpkCode = vl_poline(pick_num,line_seq)
lcCtnCode = vl_cartoncode(pick_num,line_seq)
lcPpkCode = ""
...
*- Zone D (Carrier, etc.)
lcStringBlock3=lcStringBlock3+'#T135.0#J'+str(lnOffset+45)+'#YT108/2///'+ ;
	'CARTON ______ #G'
lcStringBlock3=lcStringBlock3+'#T135.0#J'+str(lnOffset+55)+'#YT108/2///'+ ;
	'OF ________'+'#G'

*- Zone E (Contents)
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+63)+'#YT103/2///'+'Contents:'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+67)+'#YT107/2///'+'PO: '+alltrim(po_num)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+71)+'#YT107/2///'+'SKU: '+alltrim(lcSKU_UPC)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T150.0#J'+str(lnOffset+67)+'#YT107/2///'+'Quantity: '+alltrim(str(lnTotalBlockQty))+'#G'
*- Zone F (if not combined with Zone E)
lcStringBlock3 = lcStringBlock3 + '#T100.0#J'+str(lnOffset+6)+'#YT103/2///'+'Carton Code:'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T100.0#J'+str(lnOffset+18)+'#YT109/2///'+alltrim(lcCtnCode)+'#G'
if len(trim(lcPpkCode))>0 then
	lcStringBlock3 = lcStringBlock3 + '#T60.0#J'+str(lnOffset+6)+'#YT103/2///'+'Prepack Code:'+'#G'
	lcStringBlock3 = lcStringBlock3 + '#T60.0#J'+str(lnOffset+18)+'#YT109/2///'+alltrim(lcPpkCode)+'#G'
endif
...
*-----------------------------------------------------------------------------
*- FUNCTION vl_cartoncode(pick_num,line_seq)
  ...
  if used('fobname') and reccount('fobname')>0
    *- We have a range style
    lcSQLReturn = fobname.po_desc
	  ...
	  lcSQLQuery = "SELECT po_desc FROM zzoshprh WHERE ord_num="+alltrim(str(parm_ordnum))+;
				   " AND pick_num="+alltrim(str(parm_picknum))
...
PARAMETERS parm_picknum, parm_lineseq
  ...
  lcSQLQuery = "SELECT udfoordd1c FROM zzoshprd WHERE pick_num="+alltrim(str(parm_picknum))+ ;
               " AND line_seq="+alltrim(str(parm_lineseq))
  = v_SQLPrep(lcSQLQuery,'poline','')
  if used('poline') and reccount('poline')>0
    *- We have a valid range style
