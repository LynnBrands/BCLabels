...
IF alltrim(center_code)<>'D2S' THEN
  B_NAME1 = alltrim(sfsship_name)
  B_ADDR1 = alltrim(sfsship_addr1)
  B_ADDR2 = ""
  B_ADDR3 = alltrim(sfsship_addr2)
  B_CSZ = alltrim(sfsship_city)+', '+alltrim(sfsship_state)+'  '+SUBSTR(alltrim(sfsship_zip),1,5)+'  '+SUBSTR(alltrim(sfsship_country),1,2)
ELSE
  B_NAME1 = alltrim(sfsstore_name)
  B_ADDR1 = alltrim(sfsmark_addr1)
  B_ADDR2 = ""
  B_ADDR3 = alltrim(sfsmark_addr2)
  B_CSZ = alltrim(sfsmark_city)+', '+alltrim(sfsmark_state)+'  '+SUBSTR(alltrim(sfsmark_zip),1,5)+'  '+SUBSTR(alltrim(sfsmark_country),1,2)
ENDIF
IF AT('#',B_NAME1)>0 AND AT('##',B_NAME1)=0 THEN
  lipos = AT('#',B_NAME1)
  B_NAME1 = SUBSTR(B_NAME1,1,lipos-1)+'#'+SUBSTR(B_NAME1,lipos,(LEN(B_NAME1)-lipos)+1)
ENDIF
IF AT('%',B_ADDR1)>0 THEN
  lipos = AT('%',B_ADDR1)
  B_ADDR2 = SUBSTR(B_ADDR1,lipos,LEN(B_ADDR1)-lipos+1)
  B_ADDR1 = SUBSTR(B_ADDR1,1,lipos-1)
ENDIF
IF AT('#',B_ADDR1)>0 AND AT('##',B_ADDR1)=0 THEN
  lipos = AT('#',B_ADDR1)
  B_ADDR1 = SUBSTR(B_ADDR1,1,lipos-1)+'#'+SUBSTR(B_ADDR1,lipos,(LEN(B_ADDR1)-lipos)+1)
ENDIF
lcStringBlock3 = lcStringBlock3 + '#T158.0#J'+str(lnOffset+4)+'#YT106/2///'+'TO:'+'#G'
...
lcStringBlock3 = lcStringBlock3 + '#T155.0#J'+str(lnOffset+18)+'#YT106/2///'+B_ADDR3+'#G'
...
*- Zone D (Carrier, etc.)
lcStringBlock3 = lcStringBlock3 + '#T136.0#J'+str(lnOffset+33)+'#YT102/2///Carrier:'+alltrim(sfs_scaccode)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T136.0#J'+str(lnOffset+40)+'#YT105/2///Carton ____ OF ____#G'

*- Zone E (PO / Dept)

lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+65)+'#YT107/2///'+'PO: '+alltrim(po_num)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+70)+'#YT107/2///'+'SKU/UPC: '+alltrim(lcSKU_UPC)+'#G'
...
IF alltrim(sfsmark_state)='PR' THEN
	...
	ENDIF
	M_NAME = alltrim(sfsstore_name)
	M_ADDR1 = alltrim(sfsmark_addr1)
	M_ADDR2 = alltrim(sfsmark_addr2)
	M_CSZ = alltrim(sfsmark_city)+', '+alltrim(sfsmark_state)+'  '+SUBSTR(alltrim(sfsmark_zip),1,5)+'  '+SUBSTR(alltrim(sfsmark_country),1,2)

	lcStringBlock3 = lcStringBlock3 + '#T95.0#J'+str(lnOffset+4)+'#YT106/2///'+'FOR:'+'#G'
	lcStringBlock3 = lcStringBlock3 + '#T90.0#J'+str(lnOffset+9)+'#YT106/2///##'+ALLTRIM(store)+' '+ ;
		M_NAME+'#G'
	lcStringBlock3 = lcStringBlock3 + '#T90.0#J'+str(lnOffset+12)+'#YT106/2///'+M_ADDR1+'#G'
	lcStringBlock3 = lcStringBlock3 + '#T90.0#J'+str(lnOffset+15)+'#YT106/2///'+M_ADDR2+'#G'
	lcStringBlock3 = lcStringBlock3 + '#T90.0#J'+str(lnOffset+18)+'#YT106/2///'+M_CSZ+'#G'
ENDIF
