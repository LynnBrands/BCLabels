...
??? "#!A1#IMS203.2/238.0#ERN1"   && 8" wide by 6" long "Page"
...
*- Zone D (Carrier, etc.)
*- Zones C & D are not used by TJ Maxx.  Replaced with preticket / store-ready / country of origin info
&& TODO: Change the hardcoded values to calculated values
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+35)+'#YT108/2///PRETICKETED: NO  STORE READY: NO#G'
lcCountryOfOrigin = vl_getCOO(ord_num,line_seq)
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+40)+'#YT108/2///COUNTRY OF ORIGIN: '+lcCountryOfOrigin+'#G'
lcCartonSeq = alltrim(str(carton_seq))
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+45)+'#YT108/2///Carton Seq: '+ ;
	lcCartonSeq+' of '+alltrim(str(carton))+'#G'
...
ELSE 
  ...
  go top
	...
	if explarr[1]>-1 then
		...
		for lnIndbucket=2 to 9
			lcIndbucket=TRANS(lnIndbucket,"@L 99")
			if eval('siz'+lcIndbucket+'_qty')>0 and lnIndValidBucket<10
				lcStringBlock3=lcStringBlock3+ '#T'+str(180-((lnIndbucket-2)*10))+'#J'+ ;
					str(lnOffset+68)+'#YT103/2///'+ ;
					padl(allt(str(eval('siz'+lcIndbucket+'_qty'))),6)+"#G"
			endif
		endfor
	endif
endif
...
lcStringBlock3 = lcStringBlock3 + '#M1/1#T83.0#J'+str(lnOffset+12)+'#YT108/2///##'+ALLTRIM(store)+' '+ ;
...
*-----------------------------------------------------------------------------
*- FUNCTION vl_getCOO(ord_num,line_seq)
*-----------------------------------------------------------------------------
FUNCTION vl_getCOO
PARAMETERS pOrdNum, pLineSeq

	LOCAL lcSQLQuery, llCount, lcSQLReturn
	lcSQLQuery = "select top 1 "+;
		"CASE  "+;
		"	WHEN ll.udfloc1c IS NULL THEN ( "+;
		"		SELECT top 1 CASE WHEN lll.udfloc1c = '' THEN ctyl.cnty_name ELSE lll.udfloc1c END AS coo "+;
		"		FROM zzcordrd pdl  "+;
		"		JOIN zzcordrh pdh on pdh.pkey=pdl.fkey "+;
		"		JOIN zzxlocar lll on lll.location=pdh.contractor "+;
		"		JOIN zzxcntyr ctyl on lll.country=ctyl.country "+;
		"		WHERE pdl.style=d.style AND pdl.color_code=d.color_code "+;
		"		ORDER BY pdh.due_date desc "+;
		"	)		 "+;
		"	WHEN ll.udfloc1c = '' THEN cty.cnty_name "+;
		"	ELSE ll.udfloc1c "+;
		"end as coo "+;
		"from zzoordrd d "+;
		"left outer join zzoordud u		on u.ord_num=d.ORD_NUM and d.line_seq=u.line_seq "+;
		"left outer join zzcordrh ph	on u.prod_num=ph.prod_num  "+;
		"left outer join zzxlocar ll	on ll.location=ph.contractor "+;
		"left outer join zzxcntyr cty	on ll.country=cty.country "+;
		"where d.ord_num="+alltrim(str(pOrdNum))+" and d.line_seq="+alltrim(str(pLineSeq))
	= v_SQLPrep(lcSQLQuery,'coorec','')
	if used('coorec') and reccount('coorec')>0
		lcSQLReturn = coorec.coo
	else
		lcSQLReturn = 'STOCK ORDER'
	endif
RETURN lcSQLReturn
