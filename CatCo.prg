...
DO WHILE NOT EOF('CurPart2Final') 
	...
	*- second column header
	...
	DO WHILE !eof('curPart2Final') AND lnIndLineBlock2<=MaxNbLineBlock2
		...
		*- -----------
		if liListCtr < 3 then
			lcSkuList = lcSkuList + '#T200.0#J'+str(liNextOffset+50+(liListCtr*5))+'#YT107/2///'+ ;
				SKU_UPC+' - '+alltrim(str(Total_qty))+' ea#G'
			if total_qty_col2 > 0 then
				lcSkuList = lcSkuList + '#T150.0#J'+str(liNextOffset+50+(liListCtr*5))+'#YT107/2///'+ ;
					SKU_UPC_col2+' - '+alltrim(str(total_qty_col2))+' ea#G'
			endif
		endif
		*- Style
...
lcStringBlock2=lcStringBlock2+'#T33.3#J'+str(lnOffset+17+(lnIndLineBlock2*3))+'#YT106/2///'+ ;
	'Total: '+str(lnTotalBlockQty)+'#G'
*lcStringBlock2=lcStringBlock2+'#T100.0#J'+str(lnOffset+25+(lnIndLineBlock2*3))+'#YT102/2///'+ ;
*	'PRETICKETED: NO'+'#G'
...
lcStringBlock3 = lcStringBlock3 + '#T180.0#J'+str(lnOffset+15)+'#YT102/2///'+'18974  USA'+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+21)+'#YT102/2///'+'PackList##:'+alltrim(str(pick_num))+'#G'
...
ENDIF
lcStringBlock3 = lcStringBlock3 + '#T158.0#J'+str(lnOffset+2)+'#YT106/2///'+'TO: Almacenes Siman'+'#G'
...
*- Zone E (PO / Dept) --> Combined with C/D

lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+38)+'#YT108/2///'+'PO: '+alltrim(po_num)+'#G'
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+44)+'#YT108/2///'+'Style: '+alltrim(style)+'#G'
lcCountryOfOrigin = vl_getCOO(ord_num,line_seq)
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+50)+'#YT108/2///Country of Origin: '+lcCountryOfOrigin+'#G'
lcGroupName = vl_groupname(pick_num)
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+56)+'#YT108/2///Destination Country: '+lcGroupName+'#G'
lcDeliveryDate = vl_deliverydate(pick_num)
lcStringBlock3 = lcStringBlock3 + '#T200.0#J'+str(lnOffset+62)+'#YT108/2///Delivery Date: '+lcDeliveryDate+'#G'
lcCartonXofY = vl_cartonudf3(carton_num)
lcStringBlock3=lcStringBlock3+'#T200.0#J'+str(lnOffset+70)+'#YT108/2///'+lcCartonXofY+'#G'
...
*-----------------------------------------------------------------------------
*- FUNCTION vl_deliverydate(pick_num)
*- Returns calculated eta from zzoordrh
*- Michael J. Sabal, Notations, Inc. 18 Sept 2018
*-----------------------------------------------------------------------------

FUNCTION vl_deliverydate
PARAMETERS parm_pick_num

  LOCAL lcSQLQuery, llCount, lcSQLReturn

  lcSQLQuery = "SELECT case when PUB_SHIP_DATE<'2000-01-01' then END_DATE+4 else PUB_SHIP_DATE+4 end AS delivery FROM zzoordrh WHERE pick_num="+alltrim(str(parm_pick_num))

  = v_SQLPrep(lcSQLQuery,'beg','')
  if used('beg') and reccount('beg')>0
    *- Success
    lcSQLReturn = DTOC(beg.delivery)
  else
    lcSQLReturn = DTOC(DATE()+4)
  endif

RETURN alltrim(lcSQLReturn)

*-----------------------------------------------------------------------------
*- FUNCTION vl_groupname(pick_num)
*- Returns header group name from zzoordrh (udford5c)
*- Michael J. Sabal, Notations, Inc. 18 Sept 2018
*-----------------------------------------------------------------------------

FUNCTION vl_groupname
PARAMETERS parm_pick_num

  LOCAL lcSQLQuery, llCount, lcSQLReturn

  lcSQLQuery = "SELECT udford5c FROM zzoordrh WHERE pick_num="+alltrim(str(parm_pick_num))

  = v_SQLPrep(lcSQLQuery,'beg','')
  if used('beg') and reccount('beg')>0
    *- Success
    lcSQLReturn = beg.udford5c
  else
    lcSQLReturn = ""
  endif
